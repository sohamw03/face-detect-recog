...
def init_models():
    ...
    accessory_model = get_model(model_id="accesories/1", api_key=os.getenv("ROBOFLOW_API_KEY"))
...
def process_accessories(frame):
    ...
    try:
        ...
        labels = detections.data.get("class_names", [])
...
def main():
    ...
    while True:
        ...
        if state.current_state != STATE_CAPTURED:
            frame, accessories = process_accessories(frame)
            if len(accessories) > 0:
                result_msg = f"Detected accessories: {', '.join(accessories)}"
                cv2.putText( frame, result_msg, (10, 110), cv2.FONT_HERSHEY_DUPLEX, 0.7, (0, 255, 0), 2)
